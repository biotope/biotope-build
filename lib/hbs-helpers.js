...
module.exports.register = function (handlebars) {
	...
	handlebars.registerHelper('include', function (partialName, context) {
		...
		if (config.global.debug) {
			console.log(colors.blue('--- include helper: debug ---'));
			console.log(colors.green(`partialName: ${partialName}`));
			console.log(colors.green(`context.hash: ${context.hash}`));
		}
		...
		if (context.hash.hasOwnProperty('json')) {
			...
			if (config.global.debug) {
				console.log(colors.green(`jsonData: ${jsonData}`));
			}
		}
		...
		if (context.hash.hasOwnProperty('object')) {
			...
			if (config.global.debug) {
				console.log(colors.green(`objectData: ${objectData}`));
			}
		}
		...
		if (context.hash.hasOwnProperty('context')) {
			...
			if (config.global.debug) {
				console.log(colors.green(`contextData: ${contextData}`));
			}
		}
		...
		if (config.global.debug) {
			console.log(colors.green(`MERGED = OBJECT + JSON + CONTEXT + HASH: ${partialData}`));
			console.log(colors.blue('--- include helper: debug end ---'));
		}
