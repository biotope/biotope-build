...
module.exports.register = function (handlebars) {
	...
	handlebars.registerHelper('include', function (partialName, context) {
		...
		if (config.global.debug) {
			...
			console.log(colors.green(`context.hash: ${JSON.stringify(context.hash)}`));
		}
		...
		if (context.hash.hasOwnProperty('context')) {
			...
			if (config.global.debug) {
				console.log(colors.green(`contextData: ${JSON.stringify(contextData)}`));
			}
		}
		...
		if (config.global.debug) {
			console.log(colors.green(`MERGED = OBJECT + JSON + CONTEXT + HASH: ${JSON.stringify(partialData)}`));
