...
exports.parseOptions = (cliOptions) => {
    ...
    setByPriority(configFile, 'silent', cliOptions.silent, defaults_1.defaultCliOptions.silent);
    setByPriority(configFile, 'ignoreResult', cliOptions.ignoreResult, defaults_1.defaultCliOptions.ignoreResult);
    ...
    setObjectByPriority(configFile, 'componentsJson', cliOptions.componentsJson, defaults_1.defaultCliOptions.componentsJson);
    ...
    setObjectByPriority(configFile, 'serve', undefined, defaults_1.defaultConfigs.serve);
    if (!cliOptions.serve) {
        configFile.serve = undefined;
    }
    configFile.plugins = [
        ...(configFile.plugins || []),
        ...(configFile.silent ? defaults_1.defaultPlugins.filter((name) => name !== 'logger') : defaults_1.defaultPlugins).map((pluginName) => {
