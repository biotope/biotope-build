...
const getLegacyBanner = (config, legacy) => (legacy && config.legacy.require && config.legacy.require !== 'file' ? require_1.getContent(!config.debug) : '');
const createBuild = (config, legacy, input, warnings, outputFiles, addFile, removeFile, triggerBuild) => ({
    build: {
        ...
        pluginsConfig: {
            ...
            postcss: [config_1.postcss(config)],
...
exports.createPreBuilds = (config) => {
    ...
    return inputs.map((input, index) => createBuild(config, !inputsModules || index > 0, input, warnings, outputFiles, addFile, removeFile, triggerBuild));
};
