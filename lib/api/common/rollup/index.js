...
exports.createPreBuilds = (config) => {
    ...
    const inputs = [...Array(config.legacy ? 2 : 1)].map((_, index) => ({
        legacy: index !== 0,
        input: create_inputs_1.createInputs(config.project, config.extLogic, index !== 0 ? config.legacy.suffix : '', resolver_1.resolver(config.exclude.map((folder) => `${config.project}/${folder}`), false, config.extLogic)),
    }));
