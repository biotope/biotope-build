...
const createBuild = (config, legacy) => {
    ...
    const input = create_inputs_1.createInputs(config.project, config.extLogic, legacy ? config.legacy.suffix : '', resolver_1.resolver(config.exclude.map((folder) => `${config.project}/${folder}`), false, config.extLogic));
    return {
        build: {
            input,
            onwarn(warning) {
